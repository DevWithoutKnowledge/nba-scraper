...
const boxscoreSave = async (boxscore, team, gameId) => {
  ...
  for (let i = 1; i < boxscore.length - 1; i++) {
    ...
    if (boxscore[i][0] === 'Reserves') {
      continue
    } else if (boxscore[i][1] === 'Did Not Play' || boxscore[i][1] === 'Did Not Dress' || boxscore[i][1] === 'Not With Team' || boxscore[i][1] === 'Player Suspended') {
      jsonStats = {
        game_id: gameId,
        team_name: team,
